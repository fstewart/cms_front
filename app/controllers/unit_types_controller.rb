class UnitTypesController < ApplicationController
  ...
  # GET /unit_types.json
  def index
    @unit_types = UnitType.all.order('name ASC')
  ...
  def show
    @unit_type = UnitType.all.find_by url: (params[:id])
  end
  ...
  def create
    ...
    respond_to do |format|
      if @unit_type.save
        format.html { redirect_to unit_types_url, notice: 'The Organization type was successfully created.' }
  ...
  private
    ...
    def resolve_layout
	  case action_name
		  when "index", "new", "edit", "create", "show_old", "destroy"
		  "dashboard"
		  else
		  "application"
	  end
    ...
    def set_unit_type
      @unit_type = UnitType.find_by url: (params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def unit_type_params
      params.require(:unit_type).permit(:name, :url)
    end
end
