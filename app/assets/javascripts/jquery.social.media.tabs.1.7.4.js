...
(function($){
	...
	function getFeed(target,type,url,data,o,id,share,tweetId){
		var x = '#'+target,html = [],d = '',dataType = type == 'twitter' || type == 'facebook' || type == 'dribbble' ? 'json' : 'jsonp';
		jQuery.ajax({
			url: url,
			data: data,
			cache: true,
					...
					$.each(a, function(i,item){
						if(i < o.limit){
							...
							switch(type)
							{
								...
								case 'dribbble':
								d = item.created_at;
								dribbble = o.feed == 'likes' ? item.shot : item ;
								html.push('<a href="'+item.html_url+'" class="thumb"><img src="' + dribbble.images.normal + '" alt="' + dribbble.title + '" /></a>');
								html.push('<span class="meta"><span class="views">'+num(dribbble.views_count)+'</span><span class="likes">'+num(dribbble.likes_count)+'</span><span class="comments">'+num(dribbble.comments_count)+'</span></span>');
								sq = item.html_url;
								break;
